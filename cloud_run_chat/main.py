...
class ChatRequest(BaseModel):
    ...
    - history: prior conversation turns, as [(role, content), ...] tuples. Role is "human" or "ai". Defaults to [] for first-turn / stateless callers.
    """
...
def trim_history_by_turns(
    ...
    Each turn is a (role, content) tuple.
    """
    if not history:
        return []
...
def chat(req: ChatRequest):
    ...
    2. Trim chat history to max_turns
    ...
    try:
        ...
        history=trim_history_by_turns(req.history, max_turns=7)
        answer=_chain.invoke({"question": req.query, "chat_history":history,})
